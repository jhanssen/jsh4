...
- [x] **Command-line module import** — `@import path1 path2 ...` dynamically loads JS/TS modules and registers their exported functions as `@`-fns. Yields one `{from, loaded[]}` row per path. Idempotent via Node's ESM cache. Path resolution: relative to cwd or absolute. Schema extraction is async (next prompt's parse picks up schemas; same caveat as jshrc cache-miss).
...
- [x] **History dedup / ignore rules** — `HISTCONTROL` (`ignoredups`, `ignorespace`, `ignoreboth`) and `HISTIGNORE` (colon-separated globs) honored at history-append time. `erasedups` deferred (needs a native delete primitive).
...
- [x] **Stopped forkExec'd pid leak** — `handleStopped` now re-parks evicted pids in `g_orphans` so a SIGCHLD that arrives before the next `waitForPids` (e.g. an external SIGCONT racing the user's `fg`) parks the new status correctly.
- [x] **`jsh.exec` stderr iteration** — Stderr lines now feed an internal record queue. Default iterator stays stdout-only strings (backward compat). New `iterStderr()` yields stderr only; `iterAll()` yields tagged `{stream, line}` records interleaved.
- [x] **SIGCHLD refcount leak on spawn errors** (`src/native/executor.cc`) — Spawn-error paths in spawnPipeline now use `erasePending(p)` instead of bare `g_pending.erase(p)`. Previously each erased entry leaked a sigchld ref, keeping the event loop ref'd after a pipeline-spawn failure ("command not found" mid-pipeline).
- [x] **Schema cache TOCTOU between read and mtime stamp** (`src/structured/cache.ts`, `src/structured/extract/index.ts`) — Extractor now snapshots `mtimeMs` at entry and returns it; `cacheStore` accepts the captured mtime instead of stat'ing at write time. Closes a narrow race where a source edit during extraction would stamp a newer mtime than the content reflected.
- [x] **Process substitution silent on parse failure** (`src/executor/index.ts`) — `<()`, `<(garbage)` now throw a clear `process substitution: ...` error instead of returning a `/dev/fd/<n>` pointing at a never-written pipe (consumers used to hang silently).
- [x] **`@uniq` / `@uniq-by` key-order-sensitive identity** — Object keys now canonicalized recursively before JSON serialization, so `{a:1,b:2}` and `{b:2,a:1}` dedupe as the same row.
- [ ] **`@to-jsonl` / `@uniq` throw on cyclic objects** — `JSON.stringify` is unguarded. Pipeline crashes instead of yielding a useful error. Either a replacer that substitutes `"[Circular]"` or wrap in try/catch and emit a per-row error row (depends on the per-row error policy still pending elsewhere in this file).
- [ ] **`@table` column-width math counts UTF-16 code units, not display columns** — Multibyte glyphs (emoji, CJK) misalign the table. Needs a `wcwidth`-equivalent display-width function. Cosmetic but visible.
- [ ] **Completion cursor offsets may misalign on multibyte input** (`src/completion/index.ts:423-430`) — Lexer's byte offsets vs string indices used in the native splice path. Plausible bug; needs a clean repro before fixing.
- [ ] **Regex literals in unquoted lambdas** (`src/parser/lexer.ts:scanJsArg`) — `/foo|bar/` terminates the JS scan at the inner `|`/`&`/`;`. Workaround documented in `jshrc.d.ts`: wrap in `@{...}`. Real fix would need expression-position-aware regex detection (the same disambiguation JS lexers struggle with).
- [x] **`HISTIGNORE` patterns recompiled per submission** — `parseHistIgnore` now memoizes the compiled RegExp array by the raw env-var string. Recompiles only when `HISTIGNORE` is reassigned.
