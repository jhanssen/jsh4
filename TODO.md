...
- [ ] **Widget `update()` still runs `render()` during foreground execution, causing queue backlog** — `WidgetManager.updateWidget()` always invokes `widget.render()` even when `editing === false`. The repaint is suppressed but the render function's side effects (notably `jsh.exec` calls) still fire. A userland `setInterval` tick during a long-lived foreground process (vi, claude) keeps enqueueing spawn requests. Fix: when `editing === false`, either skip the render entirely or defer it until the next `start()`. Also consider not firing the user-facing `repaintFn` path at all between `onLine` and the next `start()`.
