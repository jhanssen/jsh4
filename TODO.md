...
- [ ] **`jsh.exec` parallelism is a lie** — `Promise.all([jsh.exec(a), jsh.exec(b)])` serializes. Both requests land on the single executor thread in `src/native/executor.cc` (`ExecutorState::run` pops one at a time and runs `processPipeline` synchronously). Widget code written expecting concurrency silently runs serial — a slow `git status` in `gitInfo()` blocks the `git rev-list` it's supposed to overlap with. Fix: spawn from the calling thread (posix_spawn doesn't block; only `waitpid` does), or use N executor threads.
- [ ] **Widget `update()` still runs `render()` during foreground execution, causing queue backlog** — `WidgetManager.updateWidget()` always invokes `widget.render()` even when `editing === false`. The repaint is suppressed but the render function's side effects (notably `jsh.exec` calls) still fire. A userland `setInterval` tick during a long-lived foreground process (vi, claude) keeps enqueueing spawn requests behind the foreground's `processWait` on the serialized executor thread. On exit, the backlog drains and delays the next prompt by 10s of ms per queued request (observed ~1.3s after a ~30s vi session with a 1s-tick git-status widget). Fix: when `editing === false`, either skip the render entirely or defer it until the next `start()`. Also consider not firing the user-facing `repaintFn` path at all between `onLine` and the next `start()`.
...
- [ ] **`@{ ... }` with real newlines in the body fails with a vague error** — `readJsExpression` accepts unescaped newlines inside JS string literals, but the eventual `new Function("return (${body})")` rejects them, surfacing as `jsh: @{}: Invalid or unexpected token` with no hint. Triggers whenever a user pastes a multi-line `@{ ... }` (jsh joins continuation lines with `\n` and any embedded `"..."` spans a newline). Fix: detect unescaped newlines in `readJsStringLiteral` and throw a parser error that points at the line.
